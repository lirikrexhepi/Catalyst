...
export default function AuthScreen({ onDone }: { onDone: () => void }) {
  ...
  useEffect(() => {
    setScanSupported(typeof (window as unknown as { BarcodeDetector?: unknown }).BarcodeDetector !== 'undefined')
    return () => stopScan()
  }, [])
  ...
  const startScan = async () => {
    ...
    try {
      ...
      const tick = async () => {
        ...
        try {
          ...
          if (codes.length > 0 && codes[0].rawValue) {
            await handleLink(codes[0].rawValue)
            return
          }
        } catch { /* keep scanning */ }
      ...
      setError('Camera blocked. Paste the link from your PC instead.')
    }
  }
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24, textAlign: 'center', overflowY: 'auto' }}>
      ...
      {!scanSupported && !scanning && (
        <p style={{ color: 'var(--text-mut)', fontSize: 12, maxWidth: 280, marginTop: 16 }}>
          This browser can't scan — point your camera at the PC screen, copy the link, and paste it above.
        </p>
      )}
    </div>
  )
